...
namespace AreanetFixNettoPrices\Core\Content\Product\SalesChannel\Price;
...
class ProductPriceCalculator extends AbstractProductPriceCalculator
{
    ...
    private function getPriceForTaxState(Price $price, SalesChannelContext $context): float
    {
        if ($context->getTaxState() === CartPrice::TAX_STATE_GROSS) {
            $tax = $context->getTaxRules()->get($this->product->getTaxId());

            if (!$tax instanceof TaxEntity || $tax->getRules()->count() < 1) {
                return $price->getGross();
            }

            $rule = $tax->getRules()->first();
            return $price->getNet() * ($rule->getTaxRate() / 100 + 1);
        }
